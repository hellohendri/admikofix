...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">Losses</li>
@endsection
@section('pageTitle')
<h1>Losses</h1>
@endsection
...
@endsection
@section('content')
<div class="card losses_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Kategori</th>
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.losses.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->produk_id->nama_produk??""}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->jumlah}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->kategori_id->kategori??""}}</td>
							<td class="d-none d-lg-table-cell">{{Str::limit(strip_tags($data["keterangan"]), 50, "...")}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.losses.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['losses_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['losses_allow']))
                        <a href="{{route('admin.losses.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.losses.delete")}}">
            @method('DELETE')
            @csrf
