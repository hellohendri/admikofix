...
<div class="card sales_index admikoIndex">
        ...
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-sm-table-cell">Jenis Pembayaran</th>
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Status Pembayaran</th>
							<th scope="col" class="text-nowrap d-none d-lg-table-cell">Produk</th>
							<th scope="col" class="text-nowrap d-none d-lg-table-cell">Jumlah</th>
							<th scope="col" class="d-none d-lg-table-cell">Total Harga</th>
                    ...
                    @foreach($tableData as $data)
                        <tr>
							...
							<td class="text-nowrap">{{$data->customer_id->nama??""}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->jenis_pembayaran_id->jenis_pembayaran??""}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->status_pembayaran_id->status_pembayaran??""}}</td>
							<td class="text-nowrap d-none d-lg-table-cell">{{$data->produk_id->nama_produk??""}}</td>
							<td class="text-nowrap d-none d-lg-table-cell">{{$data->jumlah}}</td>
							<td class="d-none d-lg-table-cell">Rp {{$data->total_harga}}</td>
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['sales_allow']))
                        <a href="{{route('admin.sales.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.sales.delete")}}">
            @method('DELETE')
            @csrf
