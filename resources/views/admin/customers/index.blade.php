...
@endsection
@section('content')
<div class="card customers_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
                ...
                <table class="table tableSort" style="width:100%" data-dom="ltrip">
                    <thead>
                        <tr data-sort-method='thead'>
                            ...
                            <th scope="col" class="text-nowrap d-none d-md-table-cell">No. Telp</th>
                            <th scope="col" class="text-nowrap">Email</th>
                            <th scope="col" class="text-nowrap d-none d-sm-table-cell">Alamat</th>
                            <th scope="col" class="w-5 no-sort" data-orderable="false">{{trans("admiko.table_edit")}}</th>
                            @if(Gate::allows('customers_allow'))
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="w-5"><a href="{{route("admin.customers.edit",[$data->id])}}">{{$data->id}}</a></td>
                            <td class="text-nowrap">{{$data->nama}}</td>
                            <td class="text-nowrap d-none d-md-table-cell">{{$data->no_telp}}</td>
                            <td class="text-nowrap">{{$data->email}}</td>
                            <td class="text-nowrap d-none d-sm-table-cell">{{$data->alamat}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.customers.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['customers_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['customers_allow']))
                    <a href="{{route('admin.customers.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.customers.delete")}}">
                @method('DELETE')
                @csrf
